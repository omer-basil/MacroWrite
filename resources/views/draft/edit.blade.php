<x-app-layout>
            ...
            <div class="bg-white shadow-sm">
                <div class="p-6 bg-white border-b border-gray-200">
                        ...
                        @if(Auth::user()->magazine)
                            <form  name="draftForm" action="{{ route('draft.update', $draft->magazine->id) }}" method="post" enctype = "multipart/form-data">
                                @csrf
                                <div class="relative">
                                    <div>
                                        <!-- body -->
                                        ...
                                        <input name="draftTitle" type="text" class="w-full px-4 py-3 rounded-lg shadow-sm focus:outline-none focus:shadow-outline text-gray-600 font-medium" placeholder="Your title goes here..." value="{{ $draft->title }}">
                                        @error('draftTitle')
                                            <div class="text-red-500">{{ $message }}</div>
                                        @enderror
                                    </div>
                                    <div class="mb-5">
                                        <label for="ab" class="font-bold mb-1 text-gray-700 block">Abstract</label>
                                        <textarea name="draftAbstract" type="text" class="w-full px-4 py-3 rounded-lg shadow-sm focus:outline-none focus:shadow-outline text-gray-600 font-medium resize-none" placeholder="Your title goes here..." rows="5" value="{{ $draft->abstract }}">{{ $draft->abstract }}</textarea>
                                        @error('draftAbstract')
                                            <div class="text-red-500">{{ $message }}</div>
                                        @enderror
                                    </div>
                                    <div class="flex">
                                        <label class="flex justify-start items-center text-truncate rounded-lg bg-white pl-4 pr- py-3 shadow-sm mr-4">
                                            <div class="text-teal-600 mr-3">
                                                <input name="visibility" type="radio" x-model="visibility" value="draft" class="form-radio focus:outline-none focus:shadow-outline" @if($draft->visibility == 'draft') checked @endif />
                                            </div>
                                            <div class="select-none text-gray-700">Draft</div>
                                        </label>

                                        <label class="flex justify-start items-center text-truncate rounded-lg bg-white pl-4 pr-6 py-3 shadow-sm">
                                            <div class="text-teal-600 mr-3">
                                                <input name="visibility" type="radio" x-model="visibility" value="public" class="form-radio focus:outline-none focus:shadow-outline" @if($draft->visibility == 'public') checked @endif/>
                                            </div>
                                            <div class="select-none text-gray-700">Public</div>
                                        </label>
                                    </div>
                                    @error('visibility')
                                        <div class="text-red-500">{{ $message }}</div>
    ...
    <script>
        ...
        window.onload = function() {
            ...
            document.getElementById("myeditorinstance").value += targ.textContent || targ.innerText;

            // Inserts some HTML contents at the current selection
                          
            tinymce.activeEditor.selection.setContent(targ); 
        }
