        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                ...
                <main class="py-16 container mx-auto px-6 md:px-12">
                    <section>
                    <h1 class="text-3xl font-bold text-gray-600 mb-10">Drafts you have bookmarked..</h1>
                        ...
                        @foreach($markeds as $marked)
                            <div class="w-96 h-64 py-4 px-8 bg-white shadow-lg rounded-lg my-20 relative">
                                <div class="flex justify-center md:justify-end -mt-16">
                                    <img class="w-20 h-20 object-cover rounded-full border-2 border-indigo-500" src="{{ asset('/images/' . $marked->draft->magazine->image) }}">
                                </div>
                                <div>
                                    <h2 class="text-gray-800 text-3xl font-semibold">{{ $marked->draft->title }}</h2>
                                    <p class="mt-2 text-gray-600">
                                        {{ $marked->draft->abstract }}
                                    </p>
                                    <a href="{{ route('draft.show', $marked->draft->uid) }}" class="text-red-500 inline-flex items-center m-4 absolute bottom-0 left-5">Read More
                                ...
                                <div class="flex justify-end m-4 absolute bottom-0 right-5">
                                    <a href="{{ route('mag.show', $marked->draft->magazine) }}" class="text-xl font-medium text-indigo-500">{{ $marked->draft->magazine->slug }}</a>
                                </div>
                            </div>
                        @endforeach
                    </div>
